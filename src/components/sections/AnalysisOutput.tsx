  ...
  if (!hasContent) {
    return (
      <div className="rounded border border-border bg-white p-4">
        <h2 className="text-lg font-semibold text-slate-400">Analysis Output</h2>
        <p className="text-sm text-slate-300">Run Validate to see results.</p>
      </div>
    )
  }
  ...
  return (
    <div className="rounded border border-border bg-white p-4">
      ...
      {sortedErrors.length > 0 && (
        <div className="mb-4">
          <h3 className="mb-2 text-sm font-semibold text-slate-700">Validation</h3>
          <ul className="space-y-1">
            {sortedErrors.map((e, i) => (
              <li
                key={`${e.field}-${i}`}
                className={`rounded border px-3 py-1.5 text-sm ${SEVERITY_STYLES[e.severity] ?? ''}`}
              >
      ...
      {warnings.length > 0 && (
        <div className="mb-4">
          <h3 className="mb-2 text-sm font-semibold text-slate-700">Warnings</h3>
          <ul className="space-y-1">
            {warnings.map((w, i) => (
              <li
                key={`${w.competition_id}-${w.cause}-${i}`}
                className={`rounded border px-3 py-1.5 text-sm ${SEVERITY_STYLES[w.severity] ?? ''}`}
              >
