/**
 * B1–B8 preset rosters — real USA Fencing tournament event schedules, with
 * fencer counts rounded to the nearest 10 (B8 excepted, which uses actuals).
 *
 * Relocated from `__tests__/helpers/scenarios.ts` (research.md D6) so app code
 * can offer these as presets. `__tests__/helpers/scenarios.ts` re-exports this
 * table for its existing test consumers.
 */
import type { TournamentType } from '../engine/types.ts'
