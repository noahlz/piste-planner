...
|---|---|---|
| **US5 ships three of FR-042's eight settings.** FR-042 names the admin gap, the flight buffer, the flighting threshold, the scheduling grid resolution, the per-weapon DE bout durations, the youth and veteran bout adjustment, the DE strip footprint and the pool round durations. The panel exposes the admin gap, the flight buffer and the pool round durations. | **Five settings were withheld, for two different reasons.** Four cannot move the schedule at all: T078 changed each and re-derived – `SLOT_MINS` 5→30, `YOUTH_VET_BOUT_DELTA` −5→−60, `DE_BOUT_DURATION.FOIL` 20→60, `THRESHOLD_MINS` 10→600 – and each produced a byte-identical `ScheduleResult`, so none can satisfy FR-046 without engine work constitution III puts behind the B1–B8 drift ledger. The fifth, `DEFAULT_DE_STRIP_FOOTPRINT`, was withheld on the opposite ground: it *does* move the schedule, and that is the problem. `constants.ts:68-70` records that `de_duration_table`'s empirical durations are calibrated against it, so moving it from 16 re-derives against a table valid only at 16 – measured, `de_duration_actual` 233→116. The store, `buildConfig`, serialization and engine threading for all seven keys stay in place, so only the editing surface is withheld. The destination is [`docs/design/backlog.md`](../../docs/design/backlog.md) § "A what-if scenario mode, not more settings rows", and `SettingsPanel.tsx`'s compile-time exhaustiveness check makes a future `GlobalOverrides` key declare a row or a written reason. | **Ship all nine rows and record the gap.** Rejected: a control that silently does nothing is worse than an absent one. The organizer gets no signal that six of the nine spinbuttons are inert, so the panel teaches them that settings do not work rather than that these six do not. An absent row is honest and reversible; a dead row is a defect the user discovers by losing an afternoon to it. The footprint row rejects a second alternative – **keep the settings that demonstrably move the schedule** – because a control that silently produces a plausible wrong schedule costs more than one that does nothing at all. |
