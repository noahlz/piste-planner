...
- [ ] T026 [P] [US2] Write failing tests for the category palette in `__tests__/components/canvas/palette.test.ts` – all 16 values map to a token, the four families are distinct, and lightness within a family follows the age or division ordering ([data-model.md](./data-model.md) §Category families)
- [ ] T027 [P] [US2] Write failing tests for window arithmetic in `__tests__/components/canvas/windowing.test.ts` – visible row range from scroll and viewport, visible time range from scroll and zoom, and flat-row-index to day-and-strip resolution across day groups
- [ ] T028 [P] [US2] Write failing tests for block geometry in `__tests__/components/canvas/geometry.test.ts` – x, width, y, and height derived from placement and the engine's duration math, never stored, and identical across repeated derivations (FR-013)
...
- [ ] T032 [P] [US2] Define the 16-value palette as CSS custom properties in `src/index.css` beside the existing brand tokens, with the category-to-token mapping in `src/components/canvas/palette.ts` (research D4)
- [ ] T033 [P] [US2] Draw the foil, épée, and sabre glyphs as inline SVG in `src/components/canvas/WeaponGlyphs.tsx` – lucide has no equivalents (FR-016)
- [ ] T034 [US2] Implement the time scale and hour axis as arithmetic over the visible window in `src/components/canvas/MatrixCanvas.tsx`, adding no charting dependency (research D1)
- [ ] T035 [US2] Implement row and time windowing in `src/components/canvas/MatrixCanvas.tsx` – direct index arithmetic over uniform row heights, with a day-boundary lookup for day groups. No unbounded iteration (research D2, constitution IV)
- [ ] T036 [US2] Implement the frozen strip-label gutter and sticky day header bands (FR-019)
...
- [ ] T039 [US2] Implement continuous cursor-anchored time zoom, the stepped row-height control, and the fit-to-day, fit-to-tournament, and zoom-to-selection actions, persisting through `viewState` (FR-017, FR-018, FR-020)
