...
- [x] T021 [US1] Work through the 52 cases in `__tests__/components/KitchenSinkPage.test.tsx` one at a time – re-target surviving behavior at the workbench, delete what asserted kitchen-sink composition – then delete the file (research D12) — **superseded as written, done as amended.** 005 had already deleted both files and triaged their cases, so the 52 and 27 counts are stale and no judgment calls remained. [`sessions/S3.md`](./sessions/S3.md) §"How to do T021 and T022 now" is the authority: the work reduced to re-pointing already-triaged tests at the workbench, and none of the four files needed a mount fix
- [x] T022 [US1] Work through the 27 cases in `__tests__/components/WizardShell.test.tsx` the same way – wizard navigation and step sequencing are deleted, not re-targeted – then delete the file (research D12) — **superseded as written, done as amended.** See T021
