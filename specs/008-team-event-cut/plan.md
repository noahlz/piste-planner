...
| **VI. Verified Live** | Four of ten templates go from an empty board to a real schedule — a user-visible change, so `scripts/smoke.mjs` is repaired **in place** in the task that makes it true: a team-bearing template applied and its placed count asserted at a measured number. The ROC step already there stays. |
