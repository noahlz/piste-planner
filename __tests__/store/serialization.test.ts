...
function populatedState(): StoreState {
  ...
  store.getState().setDayRefs(0, { foil_epee_refs: 6, sabre_refs: 3 })
  store.getState().setDayRefs(1, { foil_epee_refs: 5, sabre_refs: 2, allow_sabre_ref_fillin: true })
...
function validSerializedData(): SerializedState {
  return {
    ...
    referees: {
      dayRefs: [
        { foil_epee_refs: 4, sabre_refs: 2, allow_sabre_ref_fillin: false },
        { foil_epee_refs: 3, sabre_refs: 1, allow_sabre_ref_fillin: true },
        { foil_epee_refs: 5, sabre_refs: 3, allow_sabre_ref_fillin: false },
      ],
    },
  }
}
...
describe('serializeState', () => {
  it('produces JSON with schemaVersion: 1 and all serializable slice data', () => {
    ...
    expect(parsed.referees.dayRefs[1].allow_sabre_ref_fillin).toBe(true)
  })
